...
def generate_year_options():
    ...
    try:
        ...
        service_start_year = models.Service.objects.order_by('year').first().year
        year_options = [{'label': str(year), 'value': year} for year in range(service_start_year, current_year + 1)]
