...
def service_quarterly(year, month):
    ...
    service_data = {service_name[i]: {"service_name": service_name[i], "service_sla": '{:.5f}'.format(round( service_sla[i], 8)),
                                "service_count": service_count[i]} for i in range(len(service_name))}
    service_data_first_month = {service_name[i]: {"service_name": service_name[i], "service_sla": '{:.5f}'.format(round( service_sla_first_month[i], 8)),
    ...
    if len(service_sla_second_month) == 0 :
        ...
        service_data_second_month = {service_name_second_month[i]: {"service_name": service_name_second_month[i], "service_sla": '{:.5f}'.format(round( service_sla_second_month[i], 8)),
                                    "service_count": service_count_second_month[i]} for i in range(len(service_name_second_month))} 
    if len(service_sla_third_month) == 0 :
        service_data_third_month = {}
    else :
        service_data_third_month = {service_name_third_month[i]: {"service_name": service_name_third_month[i], "service_sla": '{:.5f}'.format(round( service_sla_third_month[i], 8)),
                                    "service_count": service_count_third_month[i]} for i in range(len(service_name_third_month))} 
    ...
    if len(month_list_quarterly) >= 1:
        table_data[month_list_quarterly[0]] = {
            ...
            "avg_sla": '{:.5f}'.format(round(service_sla_sum_first_month, 8)),
    ...
    if len(month_list_quarterly) >= 2:
        table_data[month_list_quarterly[1]] = {
            ...
            "avg_sla": '{:.5f}'.format(round(service_sla_sum_second_month, 8)),
    ...
    if len(month_list_quarterly) >= 3:
        table_data[month_list_quarterly[2]] = {
            ...
            "avg_sla": '{:.5f}'.format(round(service_sla_sum_third_month, 8)),
